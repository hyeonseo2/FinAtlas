...
def collect_group(
    ...
    out_dir: Path,
    max_pages: int = 0,
    product_type: str = "saving",
):
    first = client.request_page(group_no, 1, product_type=product_type)
    pages = max_pages if max_pages > 0 else client.get_max_page(first)
    if pages <= 0:
        pages = 1

    rows = [first]
    save_json(out_dir / f"page_1_{product_type}.json", first)

    for p in range(2, pages + 1):
        payload = client.request_page(group_no, p, product_type=product_type)
        save_json(out_dir / f"page_{p}_{product_type}.json", payload)
        rows.append(payload)
...
def collect_all(raw_root: Path, max_pages: int = 0):
    ...
    for group_no, group_name in cfg.groups.items():
        ...
        for product_type in ("saving", "deposit"):
            bases, options = collect_group(client, group_no, out_dir, max_pages=max_pages, product_type=product_type)
