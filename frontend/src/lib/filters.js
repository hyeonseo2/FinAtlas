function isSavingsProductByName(productMeta, bankNameMap = {}) {
...
function isSavingsProduct(productMeta, bankNameMap = {}) {
    return isSavingsProductByName(productMeta, bankNameMap);
}
function hasBankGroupFilter(rawGroup, pm, bankNameMap = {}) {
...
function hasBonusCategoryInProduct(rowProductId, category, catalog) {
...
export function filterAndSort(rows, filters, conditionCatalog = {}, productMap = {}, bankNameMap = {}) {
    const result = rows.filter((o) => {
        if (!hasBankGroupFilter(filters.group, productMap[o.product_id], bankNameMap))
            return false;
        const pm = productMap[o.product_id];
        const bankText = (pm?.company_name || bankNameMap[pm?.company_code || ""] || "").toLowerCase();
        if (filters.bank && bankText && !bankText.includes(filters.bank.toLowerCase()))
            return false;
        if (filters.term && o.save_term_months !== Number(filters.term))
            return false;
        ...
        if (o.base_rate != null && o.base_rate < minBase)
            return false;
        if (o.max_rate != null && o.max_rate < minMax)
            return false;
        if (o.expected_rate < minExp)
            return false;
        const maxDiff = filters.maxDifficulty ? Number(filters.maxDifficulty) : 100;
        if (o.difficulty_score > maxDiff)
            return false;
        if (filters.savingType && !o.rate_type.includes(filters.savingType))
            return false;
        if (!hasBonusCategoryInProduct(o.product_id, filters.conditionCategory || "", conditionCatalog))
            return false;
        return true;
    });
    switch (filters.sort) {
