...
function resolveGroupCode(pm, bankNameMap = {}) {
    if (!pm)
        return "";
...
function hasBonusCategoryInProduct(rowProductId, categories, catalog) {
...
export function filterAndSort(rows, filters, sortKey, sortDirection, conditionCatalog = {}, productMap = {}, bankNameMap = {}) {
    const result = rows.filter((o) => {
        ...
        if (filters.productType) {
            if ((pm?.product_type || "saving") !== filters.productType)
                return false;
        }
...
export function getProductGroupCode(pm, bankNameMap = {}) {
